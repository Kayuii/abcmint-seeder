...
static const int ABCMINT_SEED_VERSION = 70002;
...
class CNode {
    ...
    void EndMessage() {
        ...
        string aaaa = HexStr(vSend.begin(), vSend.end());
        printf(" version[%d] ", vSend.GetVersion());
        printf("%s: SEND %s ", ToString(you).c_str(), aaaa.c_str());
        printf("(%d bytes)\n", nSize);
    ...
    void PushVersion() {
        ...
        string ver = "/Ding-seeder:0.0.0.4/";
        uint8_t fRelayTxs = 0;
        // printf("send version message: version %d, blocks=%d, us=%s, them=%s, nLocalServices=%lld nLocalNonce=%lld nTime=%lld ver=%s \n", PROTOCOL_VERSION, nBestHeight, me.ToString().c_str(), you.ToString().c_str(), nLocalServices, nLocalNonce, nTime, ver.c_str());
...
public:
    CNode(const CService &ip, vector<CAddress> *vAddrIn) :
        you(ip), nHeaderStart(-1), nMessageStart(-1), vAddr(vAddrIn), ban(0), doneAfter(0), nVersion(0) {
        vSend.SetType(SER_NETWORK);
        vSend.SetVersion(ABCMINT_SEED_VERSION);
        vRecv.SetType(SER_NETWORK);
        vRecv.SetVersion(ABCMINT_SEED_VERSION);
    }
    bool Run() {
        bool res = true;
...
bool TestNode(const CService &cip, int &ban, int &clientV, std::string &clientSV, int &blocks, vector<CAddress> *vAddr, uint64_t &services) {
    try {
        ...
        // printf("[%20s]:%d %s!!!\n", cip.ToString().c_str(), ban, ret ? "GOOD" : "BAD");
        // printf("[%20s] REV ===> ClientVersion[%d], ClientSubVersion[%s], Height[%d], Services[%ld], %s!!!\n", cip.ToString().c_str(), clientV, clientSV.c_str(), blocks, services, ret ? "GOOD" : "BAD");
