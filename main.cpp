...
public:
    ...
    CDnsSeedOpts() :
        nThreads(96), nDnsThreads(4), ip_addr("::"), nPort(53), mbox(NULL), ns(NULL), host(NULL), tor(NULL), fUseTestNet(false), fWipeBan(false), fWipeIgnore(false), ipv4_proxy(NULL), ipv6_proxy(NULL) {
    }
...
public:
    struct FlagSpecificData {
        ...
        FlagSpecificData() :
            nIPv4(0), nIPv6(0), cacheTime(0), cacheHits(0) {
        }
    };
...
extern "C" int GetIPList(void *data, char *requestedHostname, addr_t *addr, int max, int ipv4, int ipv6) {
    ...
    while (i < max) {
        int j = i + (rand() % (size - i));
        do {
            bool ok = (ipv4 && thisflag.cache[j].v == 4) || (ipv6 && thisflag.cache[j].v == 6);
            if (ok) break;
            j++;
            if (j == size)
                j = i;
...
extern "C" void *ThreadSeeder(void *) {
    // if (!fTestNet){
    //   db.Add(CService("5wyqrzbvrdsumnok.onion", 8888), true);
    // }
    do {
